...
const getSlideIndex = ($slide) => {
    return [...$q(".sunny__item")].indexOf( $slide );
}

const prevSlide = () => {
    const index = getActiveIndex();
    const $slides = $q(".sunny__item");
    const $last = $slides[$slides.length-1];
    $last.remove();
    $list.prepend($last);
    activateSlide( $q(".sunny__item")[index] );
}
const nextSlide = () => {
    const index = getActiveIndex();
    const $slides = $q(".sunny__item");
...
const chooseSlide = (e) => {
    ...
    const $slide = e.target.closest( ".sunny__item" );
